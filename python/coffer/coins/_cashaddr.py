...
def encode(prefix,version_int,payload):
    ...
    checksum = calculate_checksum(prefix, payload,GENERATOR)
    return prefix + ':' + b32encode(payload + checksum,CHARSET)
...
def decode(address_string):
    ...
    decoded = b32decode(base32string,CHARSET)
    
    if not verify_checksum(prefix, decoded, GENERATOR):
